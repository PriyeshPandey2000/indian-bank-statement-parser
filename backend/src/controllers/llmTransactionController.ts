...
import { runLlmExtraction } from '../services/llmTransactionService';

export async function extractTransactionsLlm(req: Request, res: Response): Promise<void> {
  ...
  try {
    const result = await runLlmExtraction(id);
